...
app.get("/api/recipes/search/ingredients", async(req, res) => {
  ...
  const results = await getRecipes(ingredients, ignorePantry, ranking);
...
app.put("/recipe/forms/:id", async (req, res) => {
  try {
    const {id} = req.params;
    const { ingredients, ignorePantry, ranking } = req.body;
    const updateForm = await pool.query("UPDATE IngredientForm SET Ingredient_List = $1, Ignore_Pantry = $2, Ranking = $3 WHERE Form_ID = $4", 
      [ingredients, ignorePantry, ranking, id]
    );
